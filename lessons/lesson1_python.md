...
Писать мы будем на языке python версии 3 с использованием библиотеки pygame (библиотека в программировании - это набор готового кода для решения некой задачи).
...
И отдельно - тот же код с комментариями:
```python
#!/usr/bin/env python3
import pygame # вытаскиваем библиотеку в наш программный код
...
size = (width, height) # создаем переменную с именем size(размер). Говорим, что размер состоит у нас из двух значений:
                       # ширины и высоты. Ширину и высоту мы выше определили.
...
screen = pygame.display.set_mode(size) # создаем экран, передаем ему размер (320 на 240).
                                       # В этот момент на мониторе должен появится черный квадратик.
...
while True: # запускаем бесконечный цикл
    ...
    for event in pygame.event.get():  # забираем все события ввода с устройств ввода - мыши и клавиатуры.
        ...
        print(event)                  # печатаем событие на экран
