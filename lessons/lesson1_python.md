...
Блок кода внутри условия отображается отступами. Например код на питоне:
```python
...
```
В данном случае мы написали питону фразу `кракозябра`, он попытался понять это как имя переменной, не нашел у себя такого имени и выдал ошибку об этом. В процессе разработки игры мы столкнемся с самыми разными ошибками, я буду объяснять что они означают в процессе.
